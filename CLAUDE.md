...
- **Backoff**: exponential with full jitter — base 5 s, 2× per attempt, capped at 5 min. Retries keep going until the **cumulative wait** exceeds `execution.rate_limit_max_wait` (default 6 h), then the call returns the error. This is long enough to ride out a subscription's rolling-window cooldown (~5 h) unattended, so a pause doesn't escalate the task and derail the run; only a limit that persists past `rate_limit_max_wait` (e.g. a weekly cap) gives up. If the error message contains a `"retry after N [s|m|h]"` pattern, that wait is honored directly (capped at 24 h, then by `rate_limit_max_wait`). Detection also matches subscription-CLI limit phrasings (`usage limit`, `weekly limit`, `session limit`, `limit reached`) — lean inclusive, since a missed match derails the run while a false match only waits before failing; verify wording against the real CLIs.
